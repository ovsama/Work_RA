...
def copy_text(widget):
    if widget == result_text:
        copied_text = widget.get("1.0", "end-1c").split("FCFA")[0]  # Get text before "FCFA"
        copied_text = ''.join(c for c in copied_text if c.isdigit() or c == '.')  # Extract numeric part
    else:
        copied_text = widget.get("1.0", "end-1c")
    widget.clipboard_clear()
    widget.clipboard_append(copied_text)

...
def generate_file_structure(brand_name, dosage, pharmaceutical_form, dispatch_number):
    today = datetime.date.today().strftime('%Y-%m-%d')
    folder_name = f"{today}-Renewal-{brand_name} {dosage} {pharmaceutical_form}-Dispatch{dispatch_number}"
    ...
    open(os.path.join(m1_folder, 'M1 Local.txt'), 'w').close()
    open(os.path.join(m1_folder, 'M1 Corporate.txt'), 'w').close()
    open(os.path.join(non_ectd_folder, 'M2.txt'), 'w').close()
...
def generate_structure():
    ...
    generate_file_structure(brand_name, dosage, pharmaceutical_form, dispatch_number)
...
tk.Label(root, text="Dispatch Number:").grid(row=3, column=0, sticky="e")
dispatch_number_entry = tk.Entry(root)
dispatch_number_entry.grid(row=3, column=1)

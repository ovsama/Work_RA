...
# Create context menus for entry fields
entry_menu = tk.Menu(root, tearoff=0)
entry_menu.add_command(label="Copy", command=lambda: copy_text(root.focus_get()))
entry_menu.add_command(label="Cut", command=lambda: cut_text(root.focus_get()))
entry_menu.add_command(label="Paste", command=lambda: paste_text(root.focus_get()))
